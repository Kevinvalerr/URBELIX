...
Con la imagen real corriendo en un contenedor limitado a 512 MB (las condiciones
del plan gratuito de Render), contra un MySQL 8.0.46 vacio, perfil `prod,demo` y
`sslMode=REQUIRED`:

- Flyway aplica las cinco migraciones (`V1` a `V5`) sobre una base vacia.
- Hibernate valida el esquema resultante sin discrepancias.
- La semilla crea 24 apartamentos, 8 residentes, 49 pagos, 8 reservas,
  6 visitantes y 6 incidencias con su historial.
- Login con el administrador sembrado y navegacion correcta por `/dashboard`,
  `/pagos`, `/residentes`, `/incidencias`, `/reportes` y `/visitantes`.
- `POST /reportes/generar-pdf` devuelve un PDF valido de 2 paginas: Spring Boot
  llama al FastAPI del mismo contenedor y ReportLab lo genera.
- Consumo estable en 384 MB de los 512 MB (75 %), sin OOM.

## Pendiente de arreglar mas adelante

Varias vistas recorren asociaciones LAZY (`Pago.residente`) ya fuera de la
transaccion, asi que dependen de `spring.jpa.open-in-view=true`. Con
`open-in-view=false` el dashboard, los pagos y los reportes fallan con
`LazyInitializationException`. La solucion correcta es cargar esas relaciones
con fetch joins o proyectarlas a DTOs en los servicios; hasta entonces la
propiedad debe quedarse activada.
