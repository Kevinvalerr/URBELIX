    ...
    CommandLineRunner initData(UsuarioRepository usuarioRepository,
        ...
        return args -> {
            try {
                ...
                // Crear usuario residente de prueba y asociarlo a un apartamento
                String residenteEmail = "residente@urbelix.com";
                Usuario residenteUsuario = usuarioRepository.findByEmail(residenteEmail).orElse(null);
                if (residenteUsuario == null) {
                    residenteUsuario = new Usuario();
                    residenteUsuario.setNombre("Residente URBELIX");
                    residenteUsuario.setEmail(residenteEmail);
                    residenteUsuario.setPassword(passwordEncoder.encode("urbelix123"));
                    residenteUsuario.setRol(Rol.RESIDENTE);
                    residenteUsuario = usuarioRepository.save(residenteUsuario);
                    System.out.println("Usuario residente de prueba creado");
                }

                if (residenteRepository.findByUsuarioEmail(residenteEmail).isEmpty()) {
                    List<Apartamento> apartamentos = apartamentoRepository.findAll();
                    if (!apartamentos.isEmpty()) {
                        Residente residenteUsuarioPerfil = new Residente();
                        residenteUsuarioPerfil.setNombre(residenteUsuario.getNombre());
                        residenteUsuarioPerfil.setDocumento("90000001");
                        residenteUsuarioPerfil.setTelefono("3000000001");
                        residenteUsuarioPerfil.setApartamento(apartamentos.get(0));
                        residenteUsuarioPerfil.setUsuario(residenteUsuario);
                        residenteUsuario.setResidente(residenteUsuarioPerfil);
                        residenteRepository.save(residenteUsuarioPerfil);
                        System.out.println("Perfil residente de prueba asociado");
                    }
                }
