...
public class PagoController {
    ...
    public String rechazarPago(@PathVariable Long id, Authentication authentication, RedirectAttributes redirectAttributes) {
        try {
            Pago pago = pagoService.buscarPorId(id);
            validarAcceso(pago, authentication);
            pagoService.registrarRechazoSimulado(id);
            redirectAttributes.addFlashAttribute("error", "El pago fue rechazado en la simulación y permanece pendiente.");
        } catch (RuntimeException ex) {
            redirectAttributes.addFlashAttribute("error", "No fue posible procesar el pago.");
        }
        return "redirect:/pagos/" + id;
    }

    @GetMapping("/{id}/simular-pse")
    public String simularPse(@PathVariable Long id, Model model, Authentication authentication) {
        Pago pago = pagoService.buscarPorId(id);
        validarAcceso(pago, authentication);
        model.addAttribute("pago", pago);
        model.addAttribute("titulo", "Simulación de pago");
        model.addAttribute("currentPath", "/pagos");
    ...
    public ResponseEntity<byte[]> recibo(@PathVariable Long id, Authentication authentication) {
        Pago pago = pagoService.buscarPorId(id);
        validarAcceso(pago, authentication);
        try (ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            PdfDocument pdf = new PdfDocument(new PdfWriter(output));
            try (Document document = new Document(pdf)) {
                document.add(new Paragraph("URBELIX - RECIBO DE PAGO"));
                document.add(new Paragraph("Pago #" + pago.getId()));
                document.add(new Paragraph("Residente: " + pago.getResidente().getNombre()));
                document.add(new Paragraph("Apartamento: " + (pago.getApartamento() == null ? "N/A" : pago.getApartamento().getNumero())));
                document.add(new Paragraph("Monto: " + pago.getMonto()));
                document.add(new Paragraph("Estado: " + pago.getEstadoPago()));
                document.add(new Paragraph("Referencia: " + (pago.getReferenciaPago() == null ? "Pendiente" : pago.getReferenciaPago())));
                document.add(new Paragraph("Fecha: " + pago.getFecha()));
                document.add(new Paragraph("SIMULACIÓN ACADÉMICA - SIN TRANSACCIÓN BANCARIA REAL"));
            }
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_PDF);
            headers.setContentDisposition(ContentDisposition.attachment().filename("recibo_pago_" + id + ".pdf").build());
            return ResponseEntity.ok().headers(headers).body(output.toByteArray());
        } catch (Exception ex) {
            throw new RuntimeException("No fue posible generar el recibo", ex);
        }
    }

    private void validarAcceso(Pago pago, Authentication authentication) {
        boolean admin = authentication.getAuthorities().stream()
                .anyMatch(authority -> authority.getAuthority().equals("ROLE_ADMIN"));
        if (!admin) {
            Residente residente = residenteService.buscarPorUsuarioEmail(authentication.getName());
            if (pago.getResidente() == null || !pago.getResidente().getId().equals(residente.getId())) {
                throw new org.springframework.web.server.ResponseStatusException(
                        org.springframework.http.HttpStatus.FORBIDDEN, "No tienes permiso para ver este pago");
            }
        }
    }
}
